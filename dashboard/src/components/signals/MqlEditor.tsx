...
// `TraceStmt[]` (blank lines ignored), preserving ids BY INDEX so React keys
...
export function textToTraces(text: string, prev: TraceStmt[]): TraceStmt[] {
  const lines = text
    .split("\n")
    .map((l) => l.trim())
    .filter((l) => l !== "");
  if (lines.length === 0) {
    // Never leave a widget with zero traces — collapse to a single default.
...
export function tracesToText(traces: TraceStmt[]): string {
  return traces.map((t) => t.mql).join("\n");
}

interface MqlEditorProps {
  traces: TraceStmt[];
  onChange: (next: TraceStmt[]) => void;
}

export function MqlEditor({ traces, onChange }: MqlEditorProps) {
  ...
  // We re-sync from the incoming traces only when they describe DIFFERENT text
  ...
  // cursor.
  const [text, setText] = useState(() => tracesToText(traces));
  const incoming = useMemo(() => tracesToText(traces), [traces]);
  ...
  function handle(next: string) {
    setText(next);
    const traces = textToTraces(next, prevRef.current);
    prevRef.current = traces;
    lastEmitted.current = tracesToText(traces);
    onChange(traces);
  }

  // Keep the latest list for positional id reuse without it being a render dep.
  const prevRef = useRef(traces);
  prevRef.current = traces;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <textarea
        ...
        placeholder={"count(signal)\navg(value).where(name = \"current_ac\")\ns0 / s1"}
      />
      {lineErrors.length > 0 ? (
