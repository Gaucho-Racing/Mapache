...
func GetLatestSignalWebSocket(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			utils.SugarLogger.Errorln("[WS - gr26/live] error while reading message\n", err.Error())
			c.AbortWithError(http.StatusInternalServerError, err)
			break
		}
		utils.SugarLogger.Infoln("[WS - gr26/live] Received message ("+strconv.Itoa(messageType)+"): ", string(p))
	}
}
