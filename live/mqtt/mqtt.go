...
// signalTopicFilter is the firehose live subscribes to. No $share/ prefix —
// each live replica receives every decoded signal so its in-memory cache is
// complete and clients can land on any replica via round-robin.
const signalTopicFilter = "query/live/#"
		...
		OnConnectionUp: func(cm *autopaho.ConnectionManager, _ *paho.Connack) {
			...
			if _, err := cm.Subscribe(context.Background(), &paho.Subscribe{
				Subscriptions: []paho.SubscribeOptions{
					{Topic: signalTopicFilter, QoS: 0},
				},
			}); err != nil {
				logger.SugarLogger.Warnf("[MQ] Subscribe to %s failed: %v", signalTopicFilter, err)
			}
		},
