...
func GetSignalsForCAN(canMessageID string) ([]mapache.Signal, error) {
	rows, err := database.Conn.Query(context.Background(), `
		...
		INNER JOIN (
			SELECT vehicle_id, timestamp, node_id FROM p987_can FINAL WHERE id = ?
		) AS c
...
const insertCANSQL = `INSERT INTO p987_can (id, vehicle_id, node_id, timestamp, can_id, bytes, upload_key, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
const insertCANBatchSQL = `INSERT INTO p987_can (id, vehicle_id, node_id, timestamp, can_id, bytes, upload_key, metadata)`
