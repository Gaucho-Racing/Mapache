...
def _utc_iso(dt: datetime) -> str:
    """Render a datetime as a UTC ISO string for parseDateTime64BestEffort.

    Window bounds arrive tz-aware (built from the caller's timezone); normalise
    to UTC so the string compares correctly against the UTC `produced_at`.
    """
...
def get_signal_names(
    ...
    query_str = "SELECT DISTINCT name FROM signal WHERE vehicle_id = {vehicle_id:String}"
    if start is not None:
        query_str += " AND produced_at >= parseDateTime64BestEffort({start:String}, 6, 'UTC')"
        params["start"] = start
    if end is not None:
        query_str += " AND produced_at <= parseDateTime64BestEffort({end:String}, 6, 'UTC')"
        params["end"] = end
    query_str += " ORDER BY name ASC"

    logger.info(f"Signal names query: {query_str} | Params: {params}")
    rows = get_client().query(query_str, parameters=params).result_rows
    return [row[0] for row in rows]


def _distinct_vehicle_ids() -> list[str]:
    rows = get_client().query("SELECT DISTINCT vehicle_id FROM signal").result_rows
    return [row[0] for row in rows]
...
def _bucket_rows(
    ...
    if start is not None:
        query_str += " AND produced_at >= parseDateTime64BestEffort({start:String}, 6, 'UTC')"
        params["start"] = _utc_iso(start)
    if end is not None:
        query_str += " AND produced_at < parseDateTime64BestEffort({end:String}, 6, 'UTC')"
        params["end"] = _utc_iso(end)
    query_str += """
...
def get_data_dates(vehicle_id: str, tz: str = "UTC") -> list[str]:
    ...
    `produced_at` is a UTC DateTime64, so `toDate(produced_at, :tz)` converts
    each instant to wall-clock time in the caller's zone before truncating to a
    date. This backs the date selector: it picks the default (most recent) day
    and marks which days are selectable, without scanning per day.
    """
    if not vehicle_id:
        raise ValueError("Vehicle ID is required")
    ...
    rows = get_client().query(
        query_str,
        parameters={"vehicle_id": vehicle_id, "anchor": anchor, "tz": tz},
    ).result_rows
